...
4. **`--paused` is tri-state on `update`.** `--paused` sends `true`, `--no-paused` sends `false`, and omitting the flag sends no key at all. Because `update` is a full `PUT` replace, pass `--paused` explicitly to keep a paused schedule paused — otherwise the server applies its default and the schedule starts running again.
