...
def submit_feedback(
    ...
    details: Annotated[
        str | None,
        typer.Option("--details", help=f"Free-form details (max {DETAILS_MAX_LEN} characters)."),
    ] = None,
    project: Annotated[str | None, typer.Option("--project")] = None,
    profile: ProfileOption = None,
) -> None:
    ...
    pid = require_project(ctx, project)
    if details is not None and len(details) > DETAILS_MAX_LEN:
        emit_error(
            err(
                "DETAILS_TOO_LONG",
                f"--details is {len(details)} characters; the limit is {DETAILS_MAX_LEN}.",
                f"Shorten --details to {DETAILS_MAX_LEN} characters or fewer and re-run.",
            )
        )
